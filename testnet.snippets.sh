...
deploy() {
    DEPLOY_RESULT=$(mxpy  contract deploy --bytecode=${PROJECT} --recall-nonce --pem=${WALLET} --gas-limit=50000000 --arguments 100 2 --metadata-payable --send --proxy=${PROXY} --chain=T || return)
